...
function get_survey_employee_validate($mobile)
{
   	$emp="select emp_id as id from survey_empscore where mobile_no='$mobile'";
	$empres = mysql_query($emp);
	$emprt=mysql_fetch_assoc($empres);	
	return $emprt['id'];
}
...
function get_reinforce_employee_validate($survey,$mobile,$question_id)
{
   ...
   $emp="select emp_id as id from reinforcement_question_score where mobile_no='$mobile' and  q_id='$question_id'";
	$empres = mysql_query($emp);
	$emprt=mysql_fetch_assoc($empres);	
	return $emprt['id'];
}
...
$emp_id = get_class_employee_validate(strtoupper($message[1]),$mobile);
   if($emp_id!='')
   {
	...
	if($score=='correct')
	{
	  $response = smsob_get_quiz_correct_response("Class Training");
	}
	else if($score=='wrong')
	{
	  $response = smsob_get_quiz_incorrect_response("Class Training");
	}
	else
	{
	  $response = "You have exceeded the max attempts for this quiz, your last attempt would be considered for scoring - By AUDIIN";
	}
   }
   else
   {
    $response = "You are not added to the employee master data & hence cannot access the system - By AUDIIN";
   }
}

else if(substr($message[1], 0,1)=='s')
{
$mobile = substr($mobilenumber,-7-3);
$emp_id = get_survey_employee_validate($mobile);
   if(isset($emp_id))
   {
...
else if(substr($message[1], 0,1)=='r')
{
$mobile = substr($mobilenumber,-7-3);
$emp_id = get_reinforce_employee_validate(strtoupper($message[1]),$mobile,$message[2]);
   if(isset($emp_id))
   {
	$convet_ans = smsob_alphabet_to_integer(strtoupper($message[3]));
...
{
$response = "Invalid SMS Format, kindly check with provided template & try again - By AUDIIN";
}
