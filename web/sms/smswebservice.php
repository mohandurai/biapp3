...
{    
	if($question_id=='329' || $question_id=='330' || $question_id=='331' || $question_id=='332' || $question_id=='333' || $question_id=='334' || $question_id=='335' || $question_id=='336' || $question_id=='337' || $question_id=='338' || $question_id=='339' || $question_id=='340' || $question_id=='341' || $question_id=='342' || $question_id=='343' || $question_id=='344' || $question_id=='345' || $question_id=='20' || $question_id=='21' || $question_id=='22' || $question_id=='23' || $question_id=='24' || $question_id=='25' || $question_id=='26' || $question_id=='27' || $question_id=='28' || $question_id=='29' || $question_id=='30')
	{
	$nrw = 1;
	}
	else
...
//Employee Registration process  RSARWR G CL22
else if($message[1]=='g')
{

$mobile = substr($mobilenumber,-7-3);
   
    $sql="insert into training_reg_details(class_code, up_mobile, msg_content, emp_code)values('".$message[2]."','$mobile','".$_REQUEST['message']."','".$message[3]."')";
	$res=mysql_query($sql);
	$emp_name = $message[3];

  $sql8="select class_id from training_class where class_code='".strtoupper($message[2])."'";
  $res8=mysql_query($sql8);
  $row8 = mysql_fetch_assoc($res8);
  $class_id=$row8['class_id'];

$classcode=explode("cl",$message[2]);


  $sql6="insert into training_participant (class_id,mobile_no,emp_name,emp_id,role_id,dealership_id) values ('$class_id','$mobile','$emp_name','1','1','1')";
  $res6=mysql_query($sql6);
  
  if($res6)
  $response = "You Have been Successfully Registered for the Training class.";
