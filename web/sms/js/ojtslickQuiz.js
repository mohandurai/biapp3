...
(function($){
    $.slickQuiz = function(element, options) {
        var plugin   = this,
			...
			submitQuestionClass    = 'submitQuestion',
            tryAgainClass          = 'tryAgain',
			countQuestionClass     = 'countQuestion',
			...
			_submitQuestionBtn     = '.' + submitQuestionClass,
            _tryAgainBtn           = '.' + tryAgainClass,
			_countQuestionBtn      = '.' + countQuestionClass,
        ...
        plugin.method = {
            ...
            setupQuiz: function() {
                ...
                for (i in questions) {
                    if (questions.hasOwnProperty(i)) {
						...
						questionHTML.append('<input type="hidden" class="' +countQuestionClass+ '" id="' + question.id + '" value="' + count + '" />');
						questionHTML.append('<input type="hidden" name="question_id[]" class="' +countQuestionClass+ '" id="' + question.id + '" value="' + question.id + '" />');
                        questionHTML.append('<input type="hidden" name="num_ans[]" id="' + question.num_ans + '" value="' + question.num_ans + '" />');
                        questionHTML.append('<h5 class="quest">' + count + '. ' + question.q + '</h5>');
						...
						var answers2 = plugin.config.randomSort || plugin.config.randomSortAnswers ?
                            question.a2.sort(function() { return (Math.round(Math.random())-0.5); }) :
                            question.a2;
                        ...
                        var selectAny  = question.select_any ? question.select_any : false,
							...
							inputName2  = 'question' + (count - 1),
							
                            //inputType  = (truths > 1 && !selectAny ? 'checkbox' : 'radio'); // -- change to radio button or check box
							inputType = 'checkbox';
                        ...
                        for (i in answers) {
                            if (answers.hasOwnProperty(i)) {
                                answer   = answers[i],
								answer2   = answers2[i],
                                optionId = inputName2 + '_' + i.toString();

                                // If question has >1 true answers and is not a select any, use checkboxes; otherwise, radios
                                var input = '<input id="' + optionId + '" name="' + inputName +
                                            '[]" type="' + inputType + '" value="' + answer2.alphaoption + '" onchange="replace_check();"/>';
											
											//alert(answer2.alphaoption);
                        ...
                        if (plugin.config.backButtonText && plugin.config.backButtonText !== '') {
							if(count!=1)
							{
                            questionHTML.append('<a href="#" id="previous-btn" class="button ' + backToQuestionClass + '">' + plugin.config.backButtonText + '</a>');
							}
                        }
                        ...
                        if (plugin.config.disableResponseMessaging || plugin.config.completionResponseMessaging) {
                            //questionHTML.append('<a href="#" id="next-btn" class="button ' + nextQuestionClass + ' ' + checkAnswerClass + '">' + plugin.config.nextQuestionText + '</a>');
                        } else {
							questionHTML.append('<a href="#" id="next-btn" name="next" class="button ' + checkAnswerClass + '">' + plugin.config.checkAnswerText + '</a>');
                            //questionHTML.append('<a href="#" id="next-btn" name="submit" class="button ' + submitQuestionClass + '">' + plugin.config.submitQuestionText + '</a>');	
							questionHTML.append('<input type="submit" id="submit-btn" onclick="get_submit();" name="submit" class="button ' + submitQuestionClass + '" value="' + plugin.config.submitQuestionText + '">');	
            ...
            startQuiz: function() {
                function start() {
                    ...
                    if (firstQuestion.length) {
						...
						firstQuestion.find(_submitQuestionBtn).hide();
						if(tot==1)
						{
						firstQuestion.find(_checkAnswerBtn).hide();
						firstQuestion.find(_submitQuestionBtn).show();
						}
                    }
                }
            ...
            nextQuestion: function(nextButton) {
                var currentQuestion = $($(nextButton).parents(_question)[0]),
					...
					//alert(nextQuestion.find(_countQuestionBtn).val());
					//alert(tot);
                 var nxtcnt = nextQuestion.find(_countQuestionBtn).val();
                // If response messaging has been disabled or moved to completion,
                // make sure we have an answer if we require it, let checkAnswer handle the alert messaging
                if (plugin.config.preventUnanswered && answerInputs.length === 0) {
                    return false;
                }
				//alert(nextQuestion.length);
				//nextQuestion.find(_submitQuestionBtn).hide();
                ...
                if (nextQuestion.length) {
					...
					//nextQuestion.find(_submitQuestionBtn).show();
					
                }
				if(nxtcnt==tot)
				{
					//alert(tot);
					nextQuestion.find(_checkAnswerBtn).hide();
					nextQuestion.find(_submitQuestionBtn).show();
				}
				else
				{
					nextQuestion.find(_checkAnswerBtn).show();
					nextQuestion.find(_submitQuestionBtn).hide();
				}
            },
			/*submitQuestion: function(nextButton) {
				alert("submited!!!!!");
				 $("#quiz").submit();
        ...
        plugin.init = function() {
			...
			/*$(_element + ' ' + _submitQuestionBtn).on('click', function(e) {
                e.preventDefault();
                plugin.method.submitQuestion(this);
            });*/
