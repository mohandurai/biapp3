...
$qry1 = "SELECT category, level, parent_id, tblid, levelid, tblrowid FROM `bi_menus` WHERE `id` IN (".$menuids.") ";
//echo $qry1;
//exit;
...
//     $qry1a = "SELECT levelid FROM `bi_menus` WHERE `id` =".$prntid;
// // echo $qry1a;
// // exit;
...
//exit;
  ...
  $str2a .= "<div>";
  		...
  		$str2a .= '<input type="radio" name="level0" value="'.$aaa.'~~'.$row2['refid'].'~~'.$tblid .  '~~'. $row2['ischild'] .'" /> <label>'.$row2['label'].'</label></br>';
        ...
        '".$levelid."'  and stat = 'A' and flag = '".$opt5."' and ischild='N' ORDER BY order_fld";
          ...
          while($row3 = $res3->fetch_assoc()) 
        	{
           	$str2a .= '<div><input type="radio" name="level1" value="'.$aaa.'~~'.$row3['refid'].'~~'.$tblid . '~~'.$row3['ischild'] .'~~'. substr($tblrowid,0,-1) .'" /> <label>'.$row3['label'].'</label></div>';
        	}
