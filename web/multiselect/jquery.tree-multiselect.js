...
Tree.prototype.initialize = function () {
  ...
  // console.log('robin');
  // console.log(this.$selectionContainer);
...
Tree.prototype.generateSelections = function (parentNode) {
  // console.log(parentNode);//genertion part --robin
...
Tree.prototype.createAst = function (options) {
  ...
  options.each(function () {
    ...
    // alert(option.getAttribute('data-flag'));
    var item = Ast.createItem({
      ...
      opflag:option.getAttribute('data-opflag')
    });
    // console.log(item);
     // opflag:option.getAttribute('data-opflag')
    // console.log(sdf);
    ...
    var section_flags =item.opflag && item.opflag.length > 0 ? item.opflag.split(self.params.sectionDelimiter) : [];//robin
    // alert(sectionParts);

    for (var ii = 0; ii < sectionParts.length; ++ii) {
      var sectionPart = sectionParts[ii];
      var section_flag = section_flags[ii];//robin
      // alert(sectionPart);
...
Tree.prototype.generateHtml = function (astArr, parentNode) {
  ...
  for (var ii = 0; ii < astArr.length; ++ii) {
    ...
    if (astObj.isSection()) {
      ...
      {
        var createCheckboxes = false; //robin
      }
