...
{
window.location.assign("index.php?r=site/quizcontent&id="+contentid);
} 
...
.qz-img-responsive
{
	border:4px solid rgb(5,104,134);
}
...
<div>
                		       <?
                		             $crt=$_SESSION['$crt'];
                                     $crtidsessio= $_SESSION['$crtidsession'];
						          /*
						 ...
						 <?  $infocnt = Yii::$app->db->createCommand('SELECT crt_content,crt_title as info FROM  crt where crt_id="'.$id.'"')->queryAll();
							   for($i=0;$i<count($infocnt);$i++)
							   {
								$content= $infocnt[$i]['crt_content'];
								$info= $infocnt[$i]['info'];
								}
							   ...
							   for($m=0;$m<count($cnt);$m++)
							   {
								...
								//Image path
								 $cnttype = Yii::$app->db->createCommand('SELECT path FROM  attachments where record_id="'.$content.'"')->queryAll();
 ...
 <div  class="dh-border row">
				...
				<!--image Document-->
					<center>
					<img src="<?= $baseurl;?>" class="img-responsive survey-border" style="">
					</center>
				<!--PDF Docuemtn-->
...
<center>
<iframe src="http://docs.google.com/gview?url=<?= $baseurl1 ?>&embedded=true" style="width:100%; height:500px;" frameborder="0" class="survey-border"></iframe>
					</center>



					<? } else if($extention=='Document' or $extention=='Excel' or $extention=='ppt') { 
...
<?
 ...
 $url="http://$_SERVER[HTTP_HOST]/".$name[1]."/quiz/infonuggetsquiz.php";
