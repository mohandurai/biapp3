...
{
//alert("kkk");
$.post( " '.Url::to(["/user/geolist"]).'", { pincode:$("#user-pincode").val()}, function (data) {
//console.log(data);
...
console.log(item.state);
                                   ...
                                   $("#user-country").val(item.country);
                                     $("#user-state").val(item.state);
                                       $("#user-city").val(item.city);
                                       $("#user-territory").val(item.territory);
                                       $("#user-area").val(item.area);
                                       $("#user-zone").val(item.village);                                       
                                });
});
});
...
if(isset($_GET['id'])) {
 	$sql3 = "select role from user where id=".$_GET['id'];
	$res3 = mysql_query($sql3);
	$rt3 = mysql_fetch_assoc($res3);
	$role5 = $rt3['role'];
	$rlname = explode("_",$role5);

 	$sql33 = "select description from audi_neev_designation where neev_designation='". $rlname[1] . "'";
	$res33 = mysql_query($sql33);
	$rt33 = mysql_fetch_assoc($res33);
	//$role55 = $rt33['description'];

	$role[$rlname[1]] = $rt33['description'];
}


//$db= Yii::$app->user->identity->dp;
$role=   ArrayHelper::map(\app\models\User::find()->all(),'role','role');

?>
<div class="user-form">

   <?php $form = ActiveForm::begin(['id' => 'form-user','options'=>['enctype'=>'multipart/form-data','class' => 'form-vertical']]); ?>
...
if(Yii::$app->user->identity->role!='admin'){
        ...
        $ArrayRole=   ArrayHelper::map(\app\models\Authitem::find()->where(['type'=>1])->all(),'name','description');
}else{
$ArrayRole=   ArrayHelper::map(\app\models\Authitem::find()->where(['type'=>1])->all(),'name','name');
}

if(!is_array($model->role)&& $model->role != '')
...
$model->reports_to_role = $model->reportstouser->role;
if(!is_array($model->reports_to_role)&& $model->reports_to_role != '')
        $model->reports_to_role = explode(',', $model->reports_to_role);
?>
...
<div class="col-lg-4"> 
  ...
  <?php // $form->field($model, 'reports_to')->dropDownList($dropval1);
	...
	$userLists     =   User::find()->select('id, first_name')->where(['role' => $model->reports_to_role])->all();
	$Arrayreportusers=   ArrayHelper::map($userLists,'id','first_name');
    if(!is_array($model->reports_to)&& $model->reports_to != '')
        $model->reports_to = explode(',', $model->reports_to);
 ?>

  <?= $form->field($model, 'reports_to')->widget(DepDrop::classname(), [
        'data'=>$Arrayreportusers,
        'type' => DepDrop::TYPE_SELECT2,
        'select2Options'=>['pluginOptions'=>['allowClear'=>true]],
        'pluginOptions'=>[
            'depends'=>['user-reports_to_role'],
            'placeholder' => 'Select...',
            'url' => Url::to(['/user/reporttolist']),
            // 'value'=>$model->p_id,
       //    'params'=>['user-reports_to_role'],
            //'loadingText' => 'Loading....', 
        ]
...
if(!is_array($model->validator_role)&& $model->validator_role != '')
        $model->validator_role = explode(',', $model->validator_role);
?>
</div>
...
<?php /* = $form->field($model, 'zone')->widget(Select2::classname(), [
...
<?php /* = $form->field($model, 'state')->widget(Select2::classname(), [
...
<?php /* = $form->field($model, 'area')->widget(Select2::classname(), [
...
<?php /*= $form->field($model, 'territory')->widget(Select2::classname(), [
...
<div class="col-lg-4"> 
<?php 
    if(!is_array($model->dealership)&& $model->dealership != '')
        $model->dealership = explode(',', $model->dealership);
?>
...
?>
 ...
 <?php /* = $form->field($model, 'channel')->widget(Select2::classname(), [
...
<div class="col-lg-4"> 
 <?= $form->field($model, 'audiindiauser')->dropDownList(['0' => 'No', '1' => 'Yes' ]) ?>
 </div>
<!-- <div class="col-lg-4"> 
...
<?php 
$url = Yii::$app->urlManager->createAbsoluteUrl('site/reporttolist'); 


$this->registerJs(
    '
$("#user-reports_to_role").on("change",function(){
    
    
    $.ajax({
        url: "'.$url.'",
        type:"POST",
        data:{"reportsrole":$("#user-reports_to_role").val()},
        success: function(result){
            console.log(result);
            $("#user-reports_to").html(result);
        }
    });
});
$("#user-validator_role").on("change",function(){   
    
    $.ajax({
        url: "'.$url.'",
        type:"POST",
        data:{"reportsrole1":$("#user-validator_role").val()},
        success: function(result){
            console.log(result);
            $("#user-validator").html(result);
        }
    });
});
