...
//public $role_iddummy;
    ...
    {
        return [
        [['first_name','reports_to','role','password','username','mobile','dealership'],'required'],
            
        [['emp_code','profile_image_url','date_of_leaving','emp_code','color','date_of_joining','date_of_birth','country','zone','state','city','reports_to_role','area','territory','supervisor','trainer','qualification','co_ordinator','channel','usergroup','function','company','leader','last_name','validator_role','validator','dp','audiindiauser','department'],'safe'],
...
public function getDpuser(){
        return User::find()->andWhere(['like','role','%CEO',false]);
    }
    public function getHrUser(){
        return User::find()->andWhere(['dealership'=>$this->dealership])->andWhere(['like','role','%HRManager',false]);
    }
public function getAudiindia()
    {
        return User::find()->andWhere(['like','audiindiauser','%1%',false]);
    }
public function getRole(){
$dp = @Yii::$app->user->identity->dp;
       return $this->hasOne(AudiNeevDesignation::className(), ['role' => "concat('$dp','_','neev_designation')"])->andWhere(['mbo_frequency' => 'Monthly']);
    }
public function getRoleq(){
       return $this->hasOne(AudiNeevDesignation::className(), ['role' => 'neev_designation'])->andWhere(['mbo_frequency' => 'Quaterly']);
    }
/*
public function getRolename(){
      return $this->hasOne(AuthItem::className(),['names' => 'role'])->andWhere(['type' => 1]);
       }*/
			...
			if ($this->isNewRecord)
                        {
                          ...
                          /*  $this->created_by = Yii::$app->user->id;     
                            $this->bunit=@Yii::$app->user->identity->businessunit;*/
                        }
                        else
                        {
...
*/
    ...
    public static function findByUsername($username)            {
        {               return static::findOne(['username' => $username, 'status' => self::STATUS_ACTIVE]);
        $modelusername = static::findOne(['username' => $username, 'status' => self::STATUS_ACTIVE]);                   }
        if(!empty($modelusername) && $username===$modelusername->username)      
        {       
            return static::findOne(['username' => $username, 'status' => self::STATUS_ACTIVE]);     
        }       
        else        
        ...
        return [
            ...
            'dp' => 'Business Unit',
             ...
             'trainer'=> 'Can Be Trainer',
              ...
              'date_of_leaving'=>'Date of Resignation',
